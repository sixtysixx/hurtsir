...
async function fetchLatestVersion(binaryType, channelName) {
  ...
  const channelPath =
    channel === "LIVE" ? hostPath : `${hostPath}/channel/${channel}`;
...
async function fetchPreviousVersion(binaryType) {
  ...
  if (!response.ok) {
    throw new Error(
      `HTTP ${response.status} from ${apiBase}/api/versions/past`,
    );
  }
...
function getLink() {
  ...
  if (fieldChecked("compressZip"))
    qs += `&compressZip=true&compressionLevel=${encodeURIComponent(fieldValue("compressionLevel", "1"))}`;
...
async function dlLatest() {
  // Easy button to download the latest version of a binary!
  ...
  try {
    log(
      `[*] Resolving latest published ${binaryType} build (channel: ${channelName})...`,
    );
...
async function dlPrev() {
  ...
  try {
    log(
      `[*] Resolving previous ${binaryType} build (channel: ${channelName})...`,
    );
...
function request(url, callback, errorOnNotOk = true) {
  ...
  httpRequest.open("GET", url, true); // When the request is done later..
  ...
  httpRequest.onload = function () {
    ...
    if (errorOnNotOk && (statusCode < 200 || statusCode >= 400)) {
      log(
        `[!] Request error (${statusCode}) @ ${url} - ${httpRequest.responseText}`,
      );
      return;
    }
...
function xhrBin(url, callback, progressCallback = null) {
  ...
  httpRequest.onload = function () {
    ...
    if (!arrayBuffer) {
      log(
        `[!] Binary request error (${statusCode}) @ ${url} - Failed to get binary ArrayBuffer from response`,
      );
      return;
    }
...
async function main() {
  ...
  if (channel) {
    if (
      channel.toLowerCase() === "live" ||
      channel.toLowerCase() === "production"
    ) {
      channel = "LIVE";
    } else {
  ...
  if (version) {
    ...
    if (!version.startsWith("version-")) {
      // Only the version GUID is actually necessary
  ...
  if (version && !binaryType) {
    log(
      "[!] Error: If you provide a specific `version`, you need to set the `binaryType` aswell! See the usage doc below for examples of various `binaryType` inputs:",
      "\n\n",
    );
    log(usageMsg, "\n", false);
    return;
  ...
  if (blobDir) {
    ...
    if (!binaryType) {
      log(
        `[!] Error: Using the \`blobDir\` query without defining \`binaryType\` has been
...
    blobDir with a \`binaryType\` of either MacPlayer or MacStudio respectively`,
  ...
  if (compressZip) {
    if (compressZip !== "true" && compressZip !== "false") {
      log(
        `[!] Error: The \`compressZip\` query must be a boolean ("true" or "false"), got "${compressZip}"`,
      );
    }
  ...
  if (compressionLevel !== "") {
    try {
      ...
      log(
        `[!] Error: Failed to parse \`compressionLevel\` query: ${error}`,
        "\n\n",
      );
      log(usageMsg, "\n", false);
      return;
    ...
    if (compressionLevel > 9 || compressionLevel < 1) {
      log(
        `[!] Error: The \`compressionLevel\` query must be a value between 1 and 9, got ${compressionLevel}`,
        "\n\n",
      );
      log(usageMsg, "\n", false);
      return;
  ...
  if (!binaryType) {
    ...
    log(
      "[!] Error: Missing required \`binaryType\` query, are you using an old perm link for a specific version?",
      "\n\n",
    );
    log(usageMsg, "\n", false);
    return;
  ...
  if (binaryType in binaryTypes) {
    ...
    versionFilePath = channelPath + binaryTypeObject.versionFile; // If `blobDir` has already been defined by the user, we don't want to override it here..
    ...
    log(
      `[!] Error: \`binaryType\` given, "${binaryType}" not supported. See list below for supported \`binaryType\` inputs:`,
      "\n\n",
    );
    log(usageMsg);
    return;
  ...
  if (exploit && !version) {
    try {
      const list = await fetch(`${apiBase}/api/status/exploits`).then((r) =>
        r.json(),
      );
      const m = list.find(
        (e) => e.title.toLowerCase() === exploit.toLowerCase(),
      );
      if (m?.rbxversion) {
      ...
      log(
        `[!] Failed to fetch exploit version for "${exploit}": ${err.message}`,
      );
      logWarpHint();
      return;
    }
  ...
  if (version) {
    ...
    try {
      ...
      log(
        `[*] No version specified, resolved to latest published build: ${version}`,
      );
      fetchManifest();
    } catch (error) {
...
async function fetchManifest() {
  ...
  if (binaryType === "MacPlayer" || binaryType === "MacStudio") {
    const zipFileName =
      (binaryType == "MacPlayer" && "RobloxPlayer.zip") ||
      (binaryType == "MacStudio" && "RobloxStudioApp.zip");
    log(
      `[+] Fetching zip archive for BinaryType "${binaryType}" (${zipFileName})`,
    );
    ...
    xhrBin(
      versionPath + zipFileName,
      function (zipData) {
        const elapsed = ((Date.now() - _startTime) / 1000).toFixed(1);
        log(`done! Completed in ${elapsed}s`);
        log(
          "Thank you for using SirHurt RDD! If you have any issues, please report them at our discord server: https://discord.gg/sirhurt",
        );
        hideProgress();
      ...
      function (percentage, loaded, total) {
        setProgress(
          percentage,
          `Downloading ${zipFileName}: ${fmtBytes(loaded)} / ${fmtBytes(total)}`,
        );
      },
    );
  } else {
    log(`[+] Fetching rbxPkgManifest for ${version}@${channel}..`); // TODO: We dont support RDDs /common but should work fine since its our own R2 bucket lol?
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 403) {
          ...
          log(
            "    Roblox no longer serves rbxPkgManifest.txt or RobloxApp.zip to non-official clients,",
          );
          log(
            "    so the downloader needs a mirror/backend that proxies them.",
          );
          log(
            "    Set `window.RDD_MIRROR` (or add `?mirror=<url>`) to point RDD at your proxy.",
          );
        } else if (resp.status === 404) {
          log(
            "[!] Oh no! It seems this version has vanished like a ghost... 👻",
          );
          log("    We haven't cached this version yet.");
          log(
            "    If the Roblox Update Tracker just detected a new version, it may take a few minutes to cache it.",
          );
          log("    Try again after 1-10 minutes!");
        } else {
          log(
            `[!] Failed to fetch rbxPkgManifest: (status: ${resp.status}, err: ${(await resp.text()) || "<failed to get response from server>"})`,
          );
        }
        return;
      }
      ...
      log(
        `[!] An error occurred while fetching rbxPkgManifest: ${error.message}`,
      );
      logWarpHint();
      return;
    }
...
async function dlPackages(
  manifestBody,
  useParallel = form.parallelDownloads.checked,
) {
  const _startTime = Date.now();
  ...
  if (pkgManifestLines[0] !== "v0") {
    log(
      `[!] Error: unknown rbxPkgManifest format version; expected "v0", got "${pkgManifestLines[0]}"`,
    );
    return;
  }
  ...
  if (pkgManifestLines.includes("RobloxApp.zip")) {
    ...
    if (binaryType === "WindowsStudio64") {
      log(
        `[!] Error: BinaryType \`${binaryType}\` given, but "RobloxApp.zip" was found in the manifest!`,
      );
      return;
    }
    ...
    if (binaryType === "WindowsPlayer") {
      log(
        `[!] Error: BinaryType \`${binaryType}\` given, but "RobloxStudio.zip" was found in the manifest!`,
      );
      return;
    }
  } else {
...
`,
  ...
  const filesToDownload = pkgManifestLines.filter(
    (l) => l.includes(".") && l.endsWith(".zip"),
  );
  ...
  async function procPkg(packageName, blobData) {
    ...
    if (!(packageName in binExtractRoots)) {
      log(
        `[*] Package "${packageName}" not in extraction roots, storing at root.`,
      );
      ...
      await JSZip.loadAsync(blobData).then(async (packageZip) => {
        ...
        packageZip.forEach((path, object) => {
          ...
          fileGetPromises.push(
            object.async("arraybuffer").then((data) => {
  ...
  if (useParallel) {
    ...
    function syncProgress() {
      ...
      if (totalSize > 0) {
        ...
        setProgress(
          pct,
          `Downloading ${filesToDownload.length} packages -${fmtBytes(totalLoaded)} / ${fmtBytes(totalSize)}`,
        );
      }
    }
    ...
    for (const packageName of filesToDownload) {
      idx++;
      log(
        `[+] Fetching "${packageName}" (${idx}/${filesToDownload.length})...`,
      );
      try {
        const blobData = await fetchBin(
          versionPath + packageName,
          (loaded, total) => {
            if (total > 0)
              setProgress(
  ...
  function buildZip() {
    ...
    if (compressZip) {
      log(
        `[!] NOTE: Compressing final zip (level ${compressionLevel}/9) -this runs on the main thread and may freeze the page for a while. Most of the content is already zipped so gains are minimal.`,
      );
    }
    log(
      "Thank you for using SirHurt RDD! If you have any issues, please report them at our discord server: https://discord.gg/sirhurt",
